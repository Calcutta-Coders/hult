...
const CommentContainer = styled(Box)(({ theme, depth }: { theme?: any; depth: number }) => ({
  display: 'flex',
...
const ThreadPage: React.FC = () => {
  ...
  const toggleCollapse = useCallback((commentId: number) => {
    setCollapsedComments(prev => {
      ...
      newCollapsed.has(commentId) ? newCollapsed.delete(commentId) : newCollapsed.add(commentId);
      return newCollapsed;
    });
  }, []);
  ...
  const handleLike = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.patch(`/api/v1/forum_threads/${id}/toggle_like`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setThread(res.data);
    } catch (err) {
  ...
  const handleChill = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.patch(`/api/v1/forum_threads/${id}/toggle_chill`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setThread(res.data);
    } catch (err) {
  ...
  const handleAddComment = useCallback(async (content: string, parentId: number | null) => {
    try {
      const token = localStorage.getItem('token');
      ...
      if (parentId) {
        const addReply = (comments: Comment[]): Comment[] => comments.map(comment => {
  ...
  const CommentComponent = useCallback(
    ({ comment, depth = 0 }: { comment: Comment; depth?: number }) => {
      ...
      const handleLocalReply = useCallback(() => {
        ...
        setReplyContent('');
        setReplyingTo(null);
      }, [replyContent, comment.id, handleAddComment]);
          ...
          <Box display="flex" alignItems="center" gap={1} mb={1}>
            ...
            {comment.replies.length > 0 && (
              <Button
                ...
                endIcon={isCollapsed ? <ExpandMoreIcon fontSize="small" /> : <ExpandLessIcon fontSize="small" />}
              >
  ...
  [collapsedComments, replyingTo, toggleCollapse, handleAddComment]
);
